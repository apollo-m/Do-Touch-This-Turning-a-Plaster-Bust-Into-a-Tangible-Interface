...
For this project, an "ARDUINO UNO REV3" which got powered through a Laptop via USB was used for programming. There’s a "Capacitive Sensing Library" for Arduino that can turn two or more Arduino pins into a capacitive sensor with the help of copper foils and resistors (https://playground.arduino.cc/Main/CapacitiveSensor/). These sensors can then sense the electrical capacitance of the human body. There are also dedicated sensors available that can be bought but the approach with the copper foils offers two important advantages:
