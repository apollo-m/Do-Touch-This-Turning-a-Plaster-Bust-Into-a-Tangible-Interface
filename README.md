...
Through some basic tests we found out that an additional copper foil layer (which got connected to a ground pin of the Arduino) should get placed behind the sensors to improve the signal quality. To avoid any short circuit a layer of hot glue which acts as an isolating layer was used. However, still, the system was too unstable and the signal-to-noise ratio was too bad because of the capacitive signal inadvertedly spreading throughout the plaster even when it is fully dried. This means that Arduino's built-in capacitive sensing capabilities are not suited for this project.
